...
- O motor roda sozinho, de minuto em minuto (não é um botão que alguém aperta): ele reclassifica quando entra inscrição ou vaga nova, gera as convocações e repassa cada vaga liberada (recusa, prazo vencido, confirmação em outra unidade) para o próximo da lista de espera daquela unidade — evento "selecionada_da_lista" com ator "motor". Nunca diga ao servidor para "rodar a classificação"; o estado do motor aparece na tela Rede, no Nível Central.
